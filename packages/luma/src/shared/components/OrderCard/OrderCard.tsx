...
const OrderCardComponent: React.FC<OrderCardProps> = ({
  ...
  return (
    <div
      onClick={() => push(link)}
      className={c(
        "grid grid-cols-2 md:grid-cols-3 gap-4 relative rounded-md bg-primary bg-opacity-80 hover:bg-opacity-100 px-6 py-4 cursor-pointer before:hover:content-[''] before:hover:rounded-l-lg before:hover:w-1 before:hover:h-[100%] before:hover:bg-success-300 before:hover:absolute before:hover:bottom-0 before:hover:left-0",
    ...
    >
      {application === 'adm' ? (
        <div className="overflow-hidden text-ellipsis whitespace-nowrap text-left space-y-1">
          ...
          <p className="text-primary">{order.user.email}</p>
        </div>
      ) : null}
      ...
      <div className="overflow-hidden text-ellipsis whitespace-nowrap text-left">
        ...
        <p className="text-primary">#{order.mpPaymentId}</p>
      </div>
